...
**/
func GetIssues(restURL string, page, limit int) (issues []*Issue, pagemax int, err error) {
    
    nIssues, err := GetNbIssues(restURL)
	if err != nil {
		return nil, 0, werr.Wrapf(err, "Cannot compute nIssues")
	}
	...
	if err != nil {
		return nil, 0, werr.Wrapf(err, "Error calling postgres API: "+url)
	}
	responseData, err := ioutil.ReadAll(response.Body)
	if err != nil {
		return nil, 0, werr.Wrapf(err, "Error decoding issues data")
	}
	if err = json.Unmarshal(responseData, &issues); err != nil {
		return nil, 0, werr.Wrapf(err, "Error json Unmarshal issues data\n"+string(responseData)+"\n")
	}
	return issues, pagemax, nil
}
